...
import platform
from dashboard import GenericDashboardServer, start_daemon_server, get_system_hardware_info
...
class SAM2Daemon:
    def __init__(self):
        ...
        self.dashboard = GenericDashboardServer(title="AI Roto SAM2 System & Performance Monitor")
    ...
    def _do_process_request(self, request: dict) -> dict:
        ...
        with torch.inference_mode():
            ...
            dev_type = str(self.device).lower() if self.device else "auto"
            if "xpu" in dev_type:
                resolved_dev = "Intel GPU (XPU / OneAPI)"
            elif "cuda" in dev_type:
                resolved_dev = f"NVIDIA CUDA GPU ({hw.get('gpu_name', 'CUDA')})"
            else:
                resolved_dev = f"CPU (Intel OpenMP: {hw['torch_threads']} Threads)"
            ...
            if "CPU" in resolved_dev and hw["has_intel_gpu"]:
                ...
                """
                self.dashboard.set_advisory(advis)

            self.dashboard.set_status_card("Resolved Compute Device", resolved_dev, color="#00e5ff")
            self.dashboard.set_status_card("Loaded Checkpoint", checkpoint.name)
            self.dashboard.set_status_card("Model Config", config_name)
            self.dashboard.set_status_card("Cached Video Clip", source.name)
            self.dashboard.set_status_card("Last Inference Latency", f"{total_t:.3f}s", color="#ff9800")
